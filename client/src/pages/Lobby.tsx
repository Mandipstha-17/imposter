    ...
    <div className="w-full max-w-sm mx-auto flex flex-col gap-4 pb-32">
      ...
      {isHost && (
        ...
        >
          ...
          <p className="section-label text-center">{myPlayer?.isPlaying ? 'You will receive a role and footballer' : 'Manage the game without playing'}</p>
        </motion.div>
      )}

      {/* Players */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-panel overflow-hidden">
        <div className="px-4 py-3 flex justify-between items-center" style={{ borderBottom: '1px solid rgba(203,213,225,0.5)' }}>
      ...
      {isHost && (
        <div
          ...
          style={{ background: 'linear-gradient(to top, rgba(240,242,255,1) 55%, transparent)' }}
        >
          <div className="max-w-sm mx-auto">
            <motion.button whileTap={{ scale: 0.97 }} onClick={handleStart} className="w-full h-14 rounded-2xl font-black text-base uppercase tracking-widest text-white btn-primary animate-pulse-indigo">
              🚀 Start Match
            </motion.button>
          </div>
        </div>
